...
import { f } from "vue-ele-form-quill-editor";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        area: [{ required: true, message: "事故区域", trigger: "change" }],
        ...
        // ],
        iphone: [{ required: true, message: "联系方式", trigger: "change" }],
        createByDeptId: [
          { required: true, message: "报送部门", trigger: "change" },
        ],
  ...
  methods: {
    ...
    async Classifications() {
      this.options = await Classification("accident_area"); //事故区域
      this.options1 = await Classification("accident_nature"); //事故性质
      this.options2 = await Classification("accident_type"); //事故类别
    },
    //选择事故区域
    ChangeStatus(e) {
      this.options.map((item) => {
        if (item.dictLabel == e) {
          this.ruleForm.areaId = item.dictCode;
        }
      });
    },
    //选择事故类型
    ChangeType(e) {
      this.options2.map((item) => {
        if (item.dictLabel == e) {
          this.ruleForm.typeId = item.dictCode;
        }
      });
    },
    //选择事故性质
    ChangeNature(e) {
      this.options1.map((item) => {
        if (item.dictLabel == e) {
          this.ruleForm.natureId = item.dictCode;
        }
      });
    },
