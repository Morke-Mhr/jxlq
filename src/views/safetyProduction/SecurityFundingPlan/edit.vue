  ...
  <div class="content_add_box">
    <div class="content_add">
      <el-form
        ...
        class="demo-ruleForm"
      >
        <h2 style="text-align: center; margin: 0 0 36px">修改</h2>
...
export default {
  ...
  data() {
    return {
      has: {
        add: `@PreAuthorize("@ss.hasPermi('fundPlan:get:list')")`
      },
      isDx: true, //是否单选 子组件
      sNeed: true, //是否传值
      ...
      rules: {
        ...
        money: [{ required: true, message: '本次申报金额', trigger: 'blur' }],
  ...
  computed: {},
  created() {
    this.ruleForm.fundaccrualId = this.person
  ...
  methods: {
    ...
    async goCheck() {
      try {
        let data = this.ruleForm.fundaccrualId
