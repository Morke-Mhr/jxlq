...
export default {
  ...
  async created() {
    this.id = this.study;
    this.traindemandId = this.study.traindemandId;
    this.passStatus = this.study.passStatus;
    this.getDetails();
    // this.resultData = await getStudyVideo(this.id);
  ...
  methods: {
    ...
    async playVideoItem(index) {
      ...
      console.log('kjFileUrl', kjFileUrl)
    ...
    async onPlayerPlay(player) {
      ...
      console.log(this.lastPlayEndTime);
      ...
      if (this.relationExamId == null) {
        ...
        if (currentTime >= duration&& this.passStatus != "合格") {
          ...
          this.$confirm("您已完成培训,是否立即开始考试?", "提示", {
            ...
            .then(() => {
              this.$router.push({ path: "/examination/myExam" ,query:{trainplanId: this.id}});
            })
            .catch(() => {});
        } else {
          this.nextNeed = false;
    ...
    async playerReadied(player) {
      this.VerifyTheFirstPass = await getStudyIsSatisfy(this.traincoursewareId);
      // 找到对应元素 禁止拉动
