<template>
  ...
  <div class="app-container">
    <el-tabs type="border-card" v-model="activeName">
      <el-tab-pane label="培训学习" name="zaix">
        <onlineTrain></onlineTrain>
      </el-tab-pane>
      <el-tab-pane label="自主学习" name="zx">
        <div class="app-container">
          <study-details
            v-if="studyFlag === 'Details'"
            :study="studyId"
            @func="getMsgFormSon"
          />
          <div v-else>
            <div class="sxForm">
              ...
              >
                <!-- <el-form-item label="课件名称" prop="kjName">
                                          <el-input v-model="pageForm.kjName" placeholder="课件名称"></el-input>
                                      </el-form-item> -->
                <el-form-item label="类别" prop="pxClassId">
                  <el-select
...
import { getStudyList } from '@/api/safety/study.js'
...
import onlineTrain from '../onlineTrain/index'
...
export default {
  components: { studyDetails, onlineTrain },
  data() {
    return {
      has: {
        query: 'train:trainDemand:selfStudyVideoList'
      },
      activeName: 'zaix',
      videoFlag: false,
      //是否显示进度条
  ...
  methods: {
    ...
    handleDetail(scope) {
      this.studyFlag = 'Details'
      this.studyId = {
        pxClassId: scope.row.pxClassId,
        pxTitleId: scope.row.pxTitleId,
        traindemandId: scope.row.traindemandId,
         passStatus:scope.row.passStatus,
      }
    },
    ...
    async getAllList() {
      this.loading = true
      const res = await getStudyList(this.pageForm)
