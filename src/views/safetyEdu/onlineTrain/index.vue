...
<script>
import { getTrainList } from '@/api/safety/onlineTrain.js'
...
export default {
  components: { onlineDetails },
  data() {
    return {
      has: {
        query: 'train:trainPlan:onlineTrainingVideoList'
      },
      videoFlag: false,
      //是否显示进度条
      videoUploadPercent: '',
  ...
  methods: {
    ...
    async getAllList() {
      this.loading = true
      const res = await getTrainList(this.pageForm)
